...
const storageAPI = {
  ...
  getConnections(): Array<Connection> {
    ...
    const parsed = data.map((dict) => {
      var conn = Connection.fromJSON(dict);
      conn.active = false;
      return conn;
    });
    return parsed;
...
const state = {
  connections: [
    new Connection("duckdb_demo", "duck_db", false, "duckdb_demo"),
...
function addDefault(connections) {
  return connections.concat([
    new Connection(unconnectedLabel, "", false, null),
  ]);
}
...
function getConnectionArgument(rootGetters, data) {
  ...
  if (model instanceof LocalModel) {
    const enrichedSources = model.sources.map((source) => {
      let editor = rootGetters.editors.find(
        (editor) => editor.name === source.editor
      );
      if (!editor) {
